...
`β = sqrt(k² - k⊥²)` is taken relative to the reference frame (`β1` is the inverse frame
velocity and `βref` the reference wavevector offset); evanescent components (`β² < 0`) are
