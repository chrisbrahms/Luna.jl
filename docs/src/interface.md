...
**Number of modes (`modes`).** By default [`prop_capillary`](@ref Interface.prop_capillary) uses mode-averaged propagation in the fundamental `:HE11` mode, which is fast and accurate at low to moderate intensity. Setting `modes` to a number `N` (the first `N` `HE₁ₘ` modes) or to a tuple of mode signifiers switches to full multi-mode propagation, which captures inter-modal coupling, self-focusing and the spatial dependence of photoionisation — but is substantially more expensive, because the cost grows steeply with the number of modes (each nonlinear step evaluates overlap integrals between them). In most circumstances, mode-averaged simulations should be at least spot-checked against a multi-mode equivalent to make sure that intermodal coupling does not make a significant difference. A good starting point is to use 4 modes; increase the number as required to achieve convergence.
