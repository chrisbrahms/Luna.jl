...
Beyond `using Luna`, this example needs a few extra imports — the [Hankel.jl](https://github.com/LupoLab/Hankel.jl) package (a dependency of Luna, available as `Luna.Hankel`) for the transverse basis, and `FFTW`/`NumericalIntegration` for the post-processing:
...
The **grid** is a [`Grid.RealGrid`](@ref) as before. The transverse basis is now a *quasi-discrete Hankel transform* `q = Hankel.QDHT(R, N; dim=3)` — a radial grid of `N` points out to radius `R` — which **replaces the mode tuple**. The transform must act along the third array dimension (`dim=3`), because free-space fields carry a polarisation axis as their second dimension (see [Free-space propagation](@ref)):
...
One difference to note downstream: the output field now carries a polarisation and a transverse dimension, with `output["Eω"]` shaped `(Nω, Npol, Nk, Nz)` (here `Npol = 1`, since the refractive-index function returns a single index). The stored field is in the Hankel-transformed (spatial-frequency) domain; apply the inverse transform `q \ Eω` to recover the field as a function of radius, and use `energyfun_ω` to calculate energies in the correct units.

Other free-space geometries follow the same pattern with a different transverse basis — a [`Grid.Free2DGrid`](@ref) for one transverse dimension, or a [`Grid.FreeGrid`](@ref) for full 3D — and the same is true for two-polarisation simulations and birefringent crystals, which only differ in the refractive-index functions passed to the linear operator and normalisation. See the [Free-space propagation](@ref) section for the underlying theory and the other scripts in [`examples/low_level_interface/freespace/`](https://github.com/LupoLab/Luna.jl/tree/master/examples/low_level_interface/freespace).
