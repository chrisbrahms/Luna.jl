...
plt.subplot(1, 2, 1)
plt.pcolormesh(grid.t*1e15, xygrid.x*1e6, (Etr[:, 1, :, end])'; cmap="seismic")
...
plt.subplot(1, 2, 2)
plt.pcolormesh(grid.t*1e15, xygrid.x*1e6, (Etr[:, 2, :, end])'; cmap="seismic")
...
fig.set_size_inches(12, 6)
plt.subplot(1, 2, 1)
plt.plot(grid.t*1e15, 1e-9*(Etr[:, 1, length(xygrid.x)÷2+1, end]);
        label="Max: $(maximum(1e-9*(Etr[:, 1, length(xygrid.x)÷2+1, end])))")
plt.ylabel("E (GV/m)")
plt.legend()
plt.xlabel("Time (fs)")
plt.title("X Polarisation")
plt.subplot(1, 2, 2)
plt.plot(grid.t*1e15, 1e-9*(Etr[:, 2, length(xygrid.x)÷2+1, end]);
          label="Max: $(maximum(1e-9*(Etr[:, 2, length(xygrid.x)÷2+1, end])))")
plt.legend()
plt.xlabel("Time (fs)")
plt.title("Y Polarisation")